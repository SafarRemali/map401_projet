...
/* teste si le fichier d'identificateur f debute par un en-tete
   ...
   - ligne 1 : P1
   - suivie de zero, une ou plusieurs lignes commenéant toutes par #
...
void entete_fichier_pbm(FILE *f)
{
	...
	/* lecture des éventuelles lignes commenéant par # */
	...
	do
	{
		...
		/* lire un caractere et tester par rapport é '#' */
		char c;
		fscanf(f, "%c", &c);
		if (c=='#')
		{
...
Image calculer_masque_image(Image I){
	...
	Image J=creer_image(L,H);

	for (int y=1; y<=H; y++){
		for(int x=1; x<=L; x++){
			if(get_pixel_image(I,x,y)==NOIR && get_pixel_image(I,x,y-1)==BLANC){
				set_pixel_image(J,x,y,NOIR);
			}
			else{
				set_pixel_image(J,x,y,BLANC);
			}
		}
	}
	return J;
