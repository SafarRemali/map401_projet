...
int main(int argc, char** argv ){
    ...
    if(argc!=3){
        ...
        printf("usage: ./simplification_bezier3 <nom_fichier.contour.txt> <distance seuil> \n");
        return 1;
    }
    ...
    strcat(nom_fichier, "_bezier3.eps");
    ...
    for(int i=0; i<nb_contour; i++){
        ...
        L=simplification_Bezier3(CONT,0,CONT.taille-1,d);
        ...
        while(j<TP.taille-2){
            ...
            C.C1=TP.tab[j+1]; C.C2=TP.tab[j+2];  C.C3=TP.tab[j+3];
            fprintf(fsimp,"%.1lf %.1lf  %.1lf %.1lf  ",C.C1.x,(double)h-C.C1.y,C.C2.x,(double)h-C.C2.y);
            fprintf(fsimp,"%.1lf %.1lf curveto\n",C.C3.x,(double)h-C.C3.y);
            j=j+3;
            C.C0=TP.tab[j];
            seg=seg+1;
        }
    ...
    printf("simplification Bezier3 d = %.1lf : %d\n",d,seg);
